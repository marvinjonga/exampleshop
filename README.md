...
┌─── K8s Namespace: exampleshop-prod────────────────────────────────────────────────┐
│                                                                                   │
│  ┌─── Pod ────────────┐  ┌─── Pod ──────────────┐  ┌─── Pod ─────────────────┐    │
│  │  Products Service  │  │  Inventory Service   │  │  [ + ] Additional       │    │
│  │  :8081             │  │  :8082               │  │  Microservices          │    │
│  │  /api/v1/products  │  │  /api/v1/inventory   │  │  cart, orders...        │    │
...
│                     │                               │                             │
│            ┌────────▼──────────┐        ┌───────────▼───────────┐                 │
│            │  Shared MongoDB   │        │    Shared Kafka       │                 │
│            │  (StatefulSet)    │        │    (StatefulSet)      │                 │
│            │  Sharded by       │        │  inventory.low-stock  │                 │
│            │  storeid          │        │ inventory.out-of-stock│                 │
│            └───────────────────┘        └───────────────────────┘                 │
│                                                                                   │
└───────────────────────────────────────────────────────────────────────────────────┘
...
### Watch events in real time (local)

To watch events in real time and to do some testing you can open 3 terminals, reduce stock on the first terminal and watch messages appear when stock is low or when out of stock:
```bash

#Terminal 1 - to reduce stock
curl -X PATCH "http://localhost:8082/api/v1/inventory/prod-001/decrement?storeid=store-london" \
  -H "Content-Type: application/json" \
  -d '{"quantity":5}'

#Terminal 2 - monitor low stock
docker compose exec kafka /opt/kafka/bin/kafka-console-consumer.sh \
  --bootstrap-server kafka:9092 \
  --topic inventory.low-stock \
  --from-beginning

#Terminal 3 - monitor out of stock
docker compose exec kafka /opt/kafka/bin/kafka-console-consumer.sh \
  --bootstrap-server kafka:9092 \
